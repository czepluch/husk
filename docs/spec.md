...
| `DUE` | `DATE` for all-day. Timed values are read as `DATE-TIME` with `TZID`, as UTC (`Z`), or floating (treated as local). New timed values are written in the local zone (`TZ` when it names a zone chrono-tz knows, else the system zone; UTC when neither does) as `TZID` plus a generated `VTIMEZONE`, the form Apple Reminders shows as plain local time; a UTC value it keeps in GMT. The generated `VTIMEZONE` lists the observances of the due date's year and grows by a year when a later edit moves the due outside it; one written with `RRULE`s by Apple or Tasks.org covers every year and is preserved like any other component. A due the file holds in UTC is rewritten in the local zone when edited; `TZID` and floating forms are kept. A `TZID` chrono-tz does not know is read as local time and, once edited, written like a new value. An instant in the second pass through the hour repeated when daylight time ends cannot be named by a wall time (RFC 5545 reads one as the first pass) and is written in UTC |
...
type = "caldav"
url = "https://radicale-public.<node>.dyndns.dappnode.io/"
username = "jacob"
password.fetch = ["command", "cat", "/home/jacob/.config/dappnode-radicale/password"]
item_types = ["VTODO"]
```
...
The password sits in a mode-600 file read with `password.fetch`. A `pass` or keyring lookup needs an unlocked gpg-agent or secret service every time the systemd timer fires, which fails quietly after a reboot until something prompts for the key. A plain file in the home directory is the same trust boundary as the vdir it syncs.

husk runs `vdirsyncer sync` (configurable command; empty disables) in a background thread after every mutation, debounced so a burst becomes one run with at most one run per two seconds, and on the `s` key. When a run finishes the TUI reloads the vdir, and the bar shows `syncing`, `synced HH:MM` or the failure. The TUI also requests a sync at startup, polls the store's change stamp (vdir directory mtimes) to reload after runs it did not start, and on quit waits for a pending run to finish. Edits are applied to the task as it was displayed, so a phone edit synced in between makes the store refuse the save; the list reloads and the edit is retried by hand. A systemd user timer every five minutes is the backstop and also picks up phone edits. `vdirsyncer discover` followed by `vdirsyncer metasync` must run whenever a list is created or renamed on a phone, because `sync` alone never carries `displayname` and `color`; `husk sync --discover` runs both with the sync command's program and the terminal attached (discover asks before creating a collection), then the sync command. The Arch package ships `vdirsyncer.timer` at 15 minutes; a drop-in sets `OnUnitActiveSec=5m`. Mind that `OnUnitActiveSec` counts from the last run of `vdirsyncer.service`, and husk runs `vdirsyncer` directly, not through the service, so on a machine that never reboots the timer can sit idle forever until `systemctl --user start vdirsyncer.service` runs once and gives it a reference point. Creating a collection from the vdir side is a trap: `vdirsyncer discover` makes it on the server without a display name, and the next `metasync` under `b wins` overwrites the local `displayname` with `jacob/<uuid>`. Create new lists on a phone, or set the name on the server with a PROPPATCH before discovering.
...
Keys, vim-flavored: `j`/`k` move, `g`/`G` first/last, `Tab` switch pane, `Enter` detail (in the projects pane: focus the task list), `J`/`K` scroll the detail, `Esc` back or clear the filter, `a` add (form), `d` done (and reopen), `x` delete (confirm), `u` undo, `e` edit (form), `n` edit notes in `$EDITOR`, `t` due (same date grammar as quick add, empty clears), `p` cycle priority none, high, medium, low, `T` tags, `m` move project (picker), `/` filter, `s` sync, `c` show completed, `?` help, `q` quit. Undo covers add, change, complete, delete and move for the session (last 20); the bar says `(u undo)` after each, and a bar message fades after five seconds or on the next key. `a` opens the add form and `e` the same form prefilled from the selected task: title, due, priority, tags, project and notes in one popup. Tab or Up/Down move between fields, Left/Right cycle priority and project, Enter saves from any field, `e` on the notes row opens `$EDITOR`, and Esc discards. The title still accepts the quick-add grammar and its tokens win over the fields. The project starts at the current project view, else `default_project`, else the first project. A form or prompt that fails keeps its text so it can be fixed. In every prompt and in the filter the cursor moves with the arrows (Ctrl jumps a word) and Home/End; Backspace and Delete remove around the cursor and characters are inserted at it.
...
Done 2026-09-02: Radicale moved from the laptop to the DappNode package `radicale.public.dappnode.eth` (its own repo) behind the HTTPS portal with a Let's Encrypt certificate; the phones and vdirsyncer point at the public name, the fingerprint pinning and the local CA are gone, and the laptop server is retired (its config and collections stay as frozen copies under `~/.config/radicale` and `~/.local/share/radicale/collections`, next to `~/.config/vdirsyncer/config.bak-20260902`). Nothing in husk changed for the move.
...
- Tasks.org writes relative alarm triggers and `X-APPLE-SORT-ORDER` of its own; same rule. It also omits `STATUS` and `CALSCALE`, uses numeric UIDs, and writes timed `DUE` values one second past the minute (`T130001`) to mark "has a time". Display minutes only. A `:00` value written by another client is still shown as timed. A UTC `DUE` with no `VTIMEZONE` (as written by todoman) is the right instant on both phones, but Apple Reminders labels it GMT and shows the date in that zone.
