...
use crate::ical::codec::{self, Component, Document, Property};
...
pub fn apply(task: &Task) -> Result<Document> {
    ...
    if task.summary != original.summary {
        todo.set(Property::text_value("SUMMARY", &task.summary));
    }
    if task.description != original.description {
        match &task.description {
            Some(text) => todo.set(Property::text_value("DESCRIPTION", text)),
            None => {
    ...
    if task.tags != original.tags {
        ...
        if !task.tags.is_empty() {
            todo.set(Property::new("CATEGORIES", join_list(&task.tags)));
        }
    }
    Ok(doc)
}
...
pub fn bump(doc: &mut Document, now: DateTime<Utc>) -> Result<()> {
    let todo = doc.root.child_mut("VTODO").context("no VTODO component")?;
    let sequence: u32 = todo
        .prop("SEQUENCE")
        .and_then(|p| p.value.trim().parse().ok())
        .unwrap_or(0);
    todo.set(Property::new("SEQUENCE", (sequence + 1).to_string()));
...
fn due_property(name: &str, due: Due, existing: Option<&Property>) -> Property {
    match due {
        Due::Date(date) => {
            Property::new(name, date.format(DATE).to_string()).with_param("VALUE", "DATE")
        }
        Due::DateTime(utc) => {
            let timed = existing.filter(|p| p.value.contains('T'));
            let tzid = timed.and_then(|p| {
                p.params
                    .iter()
                    .find(|param| param.name.eq_ignore_ascii_case("TZID"))
            });
            if let Some((param, tz)) = tzid.and_then(|param| {
                Tz::from_str(param.value.trim_matches('"'))
                    .ok()
                    .map(|tz| (param, tz))
            }) {
                let wall = utc.with_timezone(&tz).naive_local().format(DATE_TIME);
                Property::new(name, wall.to_string())
                    .with_param(param.name.clone(), param.value.clone())
            } else if timed.is_some_and(|p| !p.value.ends_with('Z') && tzid.is_none()) {
                let wall = utc.with_timezone(&Local).naive_local().format(DATE_TIME);
                Property::new(name, wall.to_string())
            } else {
                Property::new(name, utc.format(UTC_DATE_TIME).to_string())
            }
        }
    }
}
...
/// In a DST gap the wall time does not exist; the earliest valid instant is
/// close enough for a due time.
...
/// Parses an RFC 5545 duration such as `-PT15M`, `P1D` or `P2W`.
pub fn parse_duration(value: &str) -> Option<TimeDelta> {
    ...
    Some(TimeDelta::seconds(if negative {
        -seconds
    } else {
        seconds
    }))
